...
public class Button {
	...
	public void grow() {
		float scale = 1.2f;
		if (!alreadyGrow) {
			changeBounds((int) (m_x - ((m_width * scale) - m_width) / 2),
					(int) (m_y - ((m_height * scale) - m_height) / 2), (int) (m_width * scale),
					(int) (m_height * scale));
			alreadyGrow = true;
		}
	}
	...
	public void paint(Graphics g) {
		...
		g.drawString(m_name, m_x + (m_width / 2) - textWidth / 2, m_y + (m_height / 2) + 10);
	}
}
